...
use halo2_proofs::{
    ...
    plonk::{
        // self,
        Advice, //BatchVerifier,
        Column, // Constraints, 
        // Expression,
        Instance as InstanceColumn, self,
...
use crate::circuit::{
    ...
    gadget::{
        add_chip::{AddChip, AddConfig, self},
        assign_free_advice,
    },
...
use halo2_gadgets::{
    ...
    // },
    poseidon::{primitives::{self as poseidon, P128Pow5T3}, Pow5Chip as PoseidonChip, Pow5Config as PoseidonConfig},
    // sinsemilla::{
...
pub struct Circuit {
    pub(crate) x: Value<pallas::Base>,
    pub(crate) y: Value<pallas::Base>,
...
impl plonk::Circuit<pallas::Base> for Circuit {
    ...
    fn synthesize(
        ...
        let hash = {
            ...
            let poseidon_hasher =
                PoseidonHash::<_,_,P128Pow5T3, _,3,2>::init(config.poseidon_chip(), layouter.namespace(|| "Poseidon init"))?;
        ...
        layouter.constrain_instance(hash.cell(), config.primary, SIMON)?;
...
mod tests {
    ...
    use halo2_gadgets::poseidon::primitives::{Hash, P128Pow5T3, Spec, ConstantLength};
    // use ff::Field;
    ...
    fn generate_circuit_instance<R: RngCore>(mut rng: R) -> (Circuit, pallas::Base) {
        ...
        let message = [pallas::Base::from(1), pallas::Base::from(12)];

        let (round_constants, mds, _): (Vec<[pallas::Base; 3]>, [[pallas::Base;3];3], [[pallas::Base;3];3]) = P128Pow5T3::constants();
        ...
        (
            Circuit {
                x: Value::known(pallas::Base::from(1)),
                y: Value::known(pallas::Base::from(12)),
